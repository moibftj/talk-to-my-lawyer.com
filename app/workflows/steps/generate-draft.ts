...
function buildPrompt(letterType: string, intakeData: Record<string, unknown>) {
  ...
  const deadlineField = intakeData["deadlineDate"] ? `Deadline: ${intakeData["deadlineDate"]}` : ""
...
${Object.keys(intakeData)
  .filter((k) => !["amountDemanded", "deadlineDate", "incidentDate"].includes(k))
  .map(fields)
  .filter(Boolean)
  .join("\n")}
${amountField}
